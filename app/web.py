...
main{{padding:4vh 0 80px}}.badge{{display:inline-flex;gap:8px;align-items:center;border:1px solid var(--line);background:var(--card);padding:8px 12px;border-radius:99px;color:var(--muted);font-size:13px}}.dot{{width:7px;height:7px;border-radius:50%;background:#32d583;box-shadow:0 0 16px #32d583}}
...
.hero-banner{{position:relative;margin-bottom:46px;border:1px solid var(--line);border-radius:clamp(20px,4vw,38px);overflow:hidden;background:#07123c;box-shadow:0 32px 100px rgba(0,40,180,.3)}}.hero-banner img{{display:block;width:100%;height:auto;aspect-ratio:16/9;object-fit:cover}}.hero-copy{{max-width:820px}}
...
@media(max-width:760px){{.grid{{grid-template-columns:1fr}}main{{padding-top:2vh}}.hero-banner{{margin-bottom:32px}}.card p{{min-height:0}}}}@media(prefers-reduced-motion:reduce){{*{{transition:none!important;animation:none!important}}}}
...
def _storefront(username: str, products: list[Product]) -> str:
    ...
    if not cards:
        cards = '<div class="empty">Каталог пока пуст. Новые товары скоро появятся.</div>'
    return _shell(f"""<header><div class="logo">LIMYZINOV<span>.</span>SHOP</div><div class="theme"><button data-theme-set="light" title="Светлая">☀</button><button data-theme-set="dark" title="Тёмная">☾</button><button data-theme-set="system" title="Системная">◐</button></div></header><main><div class="hero-banner"><img src="/static/brand/hero-banner.png" alt="Баннер LIMYZINOV SHOP" width="1680" height="945"></div><div class="hero-copy"><div class="badge"><span class="dot"></span> Магазин открыт 24/7</div><h1>Выбирай товар.<br><span class="grad">Плати как удобно.</span></h1><p class="lead">Все товары доступны с оплатой через СБП или Telegram Stars.</p><div class="actions"><a class="btn primary" href="{url}">Открыть магазин в Telegram&nbsp; ↗</a><a class="btn" href="#catalog">Смотреть каталог</a></div></div><section id="catalog"><div class="section-title"><h2>Витрина</h2><span class="badge">СБП · ⭐</span></div><div class="grid">{cards}</div></section></main><footer>© LIMYZINOV SHOP · СБП и Telegram Stars</footer>""")
