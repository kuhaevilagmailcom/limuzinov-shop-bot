...
async def menu_text(user: User) -> str:
    return (
        "🏁 <b>Лимузинов шоп</b>\n\n"
        f"👤 {user.full_name}\n"
        f"💰 Ваш баланс: <b>{money(user.balance_rub)} ₽</b>\n"
...
async def make_physical_order(callback: CallbackQuery, provider: str, product_key: str) -> None:
    ...
    async with SessionLocal() as session:
        ...
        try:
            ...
            await callback.message.answer(f"⚠️ Не удалось создать оплату.\n<code>{str(exc)[:400]}</code>")
            await callback.answer()
            return
    ...
    await callback.message.answer(
        ...
        "После подтверждения платежа бот автоматически отметит заказ оплаченным.",
...
async def pre_checkout(query: PreCheckoutQuery) -> None:
    if not query.invoice_payload.startswith("song:"):
        await query.answer(ok=False, error_message="Неизвестный заказ")
        return
    order_id = query.invoice_payload.split(":", 1)[1]
    async with SessionLocal() as session:
        order = await session.get(Order, order_id)
        if not order or order.user_id != query.from_user.id or order.status != OrderStatus.CREATED.value:
...
async def balance(message: Message) -> None:
    ...
    await message.answer(
        "💰 <b>Ваш профиль</b>\n\n"
        f"Баланс: <b>{money(user.balance_rub)} ₽</b>\n"
        f"Всего покупок: <b>{user.purchases_count}</b>"
    )
